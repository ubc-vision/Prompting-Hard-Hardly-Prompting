...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
        ...
        self.log_steps = [n for n in range(int(self.batch_freq))]#[2 ** n for n in range(int(np.log2(self.batch_freq)) + 1)]
...
class Graphlogger(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        sample_imgs = torch.rand(1,1,32,32)
        pl_module.logger.experiment.add_graph(pl_module,sample_imgs)
...
class ModeSwapCallback(Callback):
    ...
    def __init__(self, swap_step=20000):
        super().__init__()
...
if __name__ == "__main__":
    ...
    try:
        ...
        #onfig.data.params.validation.image_id = opt.validation.image_id
